...
| Approach | 사용자가 회원정보를 입력하면 시스템은 해당 정보를 DB에 저장하고 계정을 생성 |
| Dynamics | 회원가입 요청 |
| Goals | 사용자 등록 기능 구현 |
...
| Approach | 사용자가 앱 실행 후 ID와 PW를 입력하면 시스템은 DB에서 사용자 정보를 조회하여 로그인 여부를 판단 |
| Dynamics | 앱 실행 → 로그인 시도 |
| Goals | 사용자 인증 기능 구현 |


## 3) Create Group
사용자가 공동 지출 관리를 위한 그룹을 생성한다.

| 항목 | 내용 |
|------|------------|
| Actor | User |
| Purpose | 공동 지출 관리를 위한 그룹 생성 |
| Approach | 사용자가 그룹 정보를 입력하면 시스템은 그룹을 생성하고 DB에 저장한다. |
| Dynamics | 그룹 생성 요청 발생 |
| Goals | 그룹 관리 기능 구현 |


## 4) Join Group
사용자가 기존 공동 지출 그룹에 참여한다.

| 항목 | 내용 |
|------|------------|
| Actor | User |
| Purpose | 기존 그룹에 참여 |
| Approach | 사용자가 초대 코드를 입력하면 시스템은 해당 그룹을 조회하여 참여를 승인한다. |
...
| Actor | User |
| Purpose | 공동 지출 내역 기록 |
| Approach | 사용자가 지출 정보를 입력하면 시스템은 해당 데이터를 DB에 저장한다. |
...
| Actor | User |
| Purpose | 기존 지출 정보 수정 |
| Approach | 사용자가 수정 요청을 하면 시스템은 해당 데이터를 업데이트한다. |
...
| Approach | 사용자가 삭제 요청을 하면 시스템은 해당 데이터를 DB에서 제거한다. |
...
| Approach | 시스템은 DB에서 지출 데이터를 조회하여 사용자에게 제공한다. |
| Dynamics | 조회 요청 발생 |
| Goals | 지출 조회 기능 구현 |
...
| Actor | User |
| Purpose | 정산 계산 수행 |
| Approach | 사용자가 정산 요청을 하면 시스템은 지출 데이터를 기반으로 정산 금액을 계산한다. |
...
| Actor | User |
| Purpose | 정산 결과 확인 |
| Approach | 시스템은 계산된 정산 결과를 사용자에게 제공한다. |
...
사용자가 주요 이벤트에 대한 알림을 수신한다.

| 항목 | 내용 |
|------|------------|
| Actor | User |
| Purpose | 주요 이벤트 알림 제공 |
| Approach | 시스템은 그룹 초대, 지출 변경, 정산 완료 시 Notification Service를 통해 알림을 전송한다. |
